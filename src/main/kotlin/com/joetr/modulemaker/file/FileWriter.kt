...
class FileWriter(
    ...
    private fun addToSettingsAtCorrectLocation(
        ...
        val includeKeywords =
            if (preferenceService.preferenceState.includeProjectKeyword.isNotEmpty()) {
        ...
        val lastNonEmptyLineInSettingsGradleFile =
            settingsFile.lastOrNull { settingsFileLine ->
                settingsFileLine.isNotEmpty() &&
                    includeKeywords.any {
                        settingsFileLine.contains(it)
                    }
            }
        val projectIncludeKeyword =
            includeKeywords.firstOrNull { includeKeyword ->
        ...
        val usesTwoParameters =
            settingsFile.any { line ->
                twoParametersPattern.containsMatchIn(line)
            }

        // get the last line numbers for an include statement
        val lastLineNumberOfFirstIncludeProjectStatement =
            settingsFile.indexOfLast {
        ...
        while (tempIndexForSettingsFile >= 0) {
            ...
            if (currentLine.trim().isEmpty() ||
                settingsFileContainsSpecialIncludeKeyword(
                    currentLine,
        ...
        val insertionIndex =
            includeProjectStatements.indexOfFirst {
        ...
        if (insertionIndex < 0) {
             ...
             */
            val offsetAmount =
                if (includeProjectStatements.size == 1 &&
                    includeProjectStatements
                        .first()
...
private fun String.doesNotContainModule(includeKeyword: String): Boolean = this.replace(" ", "").replace("(", "") == includeKeyword
